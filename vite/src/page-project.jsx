...
// R22 Item #2 — Per-school stage checkmark table. One row per school, 18 stage
// columns. Cell shows a green CheckCircle when the stage has a completedDate,
// otherwise a small slate placeholder dot. Sticky thead + sticky first column.
// Toolbar: free-text search by school name, plus a Completed/In progress filter.
function ProjectStageChecklistTable({ schools, activeStage, onClearStage }) {
  ...
  const filtered = React.useMemo(() => {
    ...
    if (statusFilter === 'completed') {
      ...
      rows = rows.filter(s => s.status === 'In Progress');
    }
    if (activeStage != null) {
      rows = rows.filter(s => {
    ...
    return rows;
  }, [schools, lowered, q, statusFilter, activeStage]);

  return (
    <Card padding="p-0">
      <div className="p-4 border-b border-soft flex flex-wrap items-center gap-3">
        <div className="flex-1 min-w-[220px]">
          <div className="text-sm font-semibold ink-on-dark">Per-school stage progress</div>
        ...
        {activeStage != null && (
          <span className="inline-flex items-center gap-2 px-2.5 py-1 rounded-full text-[11px] font-medium"
            ...
            S{String(activeStage + 1).padStart(2, '0')} · {SCHOOL_STAGES[activeStage]}
      ...
      <div data-testid="project-stage-checklist-table"
        className="overflow-auto scrollbar-thin" style={{ maxHeight: 480 }}>
        <table className="text-xs" style={{ borderCollapse: 'separate', borderSpacing: 0 }}>
          <thead>
            <tr>
              <th className="text-left px-3 py-2 font-semibold whitespace-nowrap surface-2 border-b border-soft"
          ...
          <tbody>
            ...
            {filtered.length === 0 && (
              <tr><td colSpan={19} className="text-center py-8 text-xs text-ink-500 italic">
