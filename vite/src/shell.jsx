...
// R30.14: filter sidebar items based on Settings → Roles & Permissions matrix
// Matrix features: Dashboard, Projects, Materials, Financials, Contractors, Reports, Employees, Settings
const SIDEBAR_ITEM_TO_FEATURE = {
  'home':           'Dashboard',
  ...
  'settings':       'Settings',
  // Items NOT in the matrix are always shown (role-driven only):
  // escalations, my-escalations, delivery-notes, audit-log, tasks, my-schools
};
function applyMatrix(items, role, rolePermissions) {
  if (!rolePermissions || !rolePermissions[role]) return items;
  const perms = rolePermissions[role];
  return items.filter(it => {
    const feature = SIDEBAR_ITEM_TO_FEATURE[it.id];
    if (!feature) return true;  // not matrix-controlled — always show
